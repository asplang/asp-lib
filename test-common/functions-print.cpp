//
// Asp common test application functions implementation: print.
//

#include "asp.h"
#include <stdio.h>

static AspRunResult print1(AspEngine *, AspDataEntry *);

/* print(*values, sep, end)
 * Print values to standard output.
 * Separate individual items with the sep value and finish with the end value.
 */
extern "C" AspRunResult AspTest_print
    (AspEngine *engine,
     ...
     AspDataEntry **returnValue)
{
    ...
    for (int32_t i = 0; i < argCount; i++)
    {
        if (i != 0)
        {
            result = print1(engine, sep);
            if (result != AspRunResult_OK)
                return result;
        }

        AspDataEntry *value = AspElement(engine, values, i);
        result = print1(engine, value);
        if (result != AspRunResult_OK)
            return result;
    }

    result = print1(engine, end);
    if (result != AspRunResult_OK)
        return result;
